...
4. SQLite validates candidates, records provenance/audit, deduplicates and resolves only truly single-value conflicts. A new response-length preference or current goal supersedes an older active value. In balanced mode only high-confidence, durable normal facts are auto-activated; sensitive facts remain in review.
