...
Провайдеры LLM для старта обсуждаются: DeepSeek, Mistral, Gemini, возможно позже OpenAI/Anthropic/Ollama/LM Studio. Ключевой принцип: провайдер‑независимая архитектура. Персонаж и dev‑agent должны использовать разные модели/роли, чтобы не нагружать одну модель и можно было подбирать модель под задачу.
...
| STT | faster-whisper локально или cloud STT | Локально приватнее, облако качественнее |
| TTS | Edge TTS / Piper / ElevenLabs | Edge — быстро и дешево, ElevenLabs — качество, Piper — локально |
...
SQLite достаточно. PostgreSQL понадобится, когда появится несколько пользователей, облачная версия, team accounts, тяжелые логи или SaaS.
