...
Тогдашняя система памяти V0.5 (`apps/backend/app/memory/service.py`) использовала детерминированное извлечение на regex-паттернах. Она умела запоминать имя, предпочтения и явные инструкции (`запомни:`), но не понимала **отношения между сущностями** и не умела отвечать на вопросы вида «кто такой X», «что связывает X и Y», «когда произошло Z».
