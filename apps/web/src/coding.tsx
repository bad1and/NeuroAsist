...
export function CodingAgentPage({
  ...
  const clearTaskList = async () => {
    ...
    if (!window.confirm("Очистить список завершённых задач? Рабочие папки и созданные файлы сохранятся.")) return;
    setBusy(true);
    try {
  ...
  return (
    <section className="coding-page" aria-labelledby="coding-agent-title">
      <header className="coding-page-header">
        ...
        <button className="secondary-button" type="button" onClick={() => void refresh(true)} disabled={busy}><RefreshCw size={16} /> Обновить среду</button>
      </header>
      ...
      <div className="coding-status-grid">
        ...
        <article><span>Песочница Docker</span><strong className={status?.available ? "is-success" : "is-danger"}>{status?.available ? "Готова" : "Недоступна"}</strong><small>{status?.availability_reason ?? "Команды выполняются без сети в копии проекта"}</small></article>
        <article><span>Текущая работа</span><strong>{active ? statusLabel(active.status) : "Нет задачи"}</strong><small>{active ? active.objective.slice(0, 92) : "Создайте задачу ниже"}</small></article>
      </div>
      ...
      <div className="coding-workspace-grid">
        ...
        <section className="coding-task-list"><h2>Задачи</h2>{tasks.length === 0 && <p className="muted">Задач пока нет.</p>}{tasks.map((task) => <button type="button" key={task.id} className={`coding-task-row${task.id === selectedId ? " is-selected" : ""}`} onClick={() => setSelectedId(task.id)}><span className={`coding-task-status status-${task.status}`}>{statusLabel(task.status)}</span><strong>{task.objective}</strong><small>{new Date(task.updated_at).toLocaleString()}</small></button>)}</section>
      </div>
      {selected && <section className="coding-detail"><header><div><span className={`coding-task-status status-${selected.status}`}>{statusLabel(selected.status)}</span><h2>{selected.objective}</h2><small>{selected.model} · {selected.project_root || "отдельная рабочая папка"}</small>{selected.workspace_path && <small>Изолированный workspace: {selected.workspace_path}</small>}</div><div className="coding-actions">{ACTIVE_TASKS.has(selected.status) && <button className="danger-button" type="button" disabled={busy} onClick={() => void act(() => cancelCodingTask(selected.id))}><Square size={15} /> Остановить</button>}{["failed", "cancelled", "waiting_for_input", "conflicted"].includes(selected.status) && <button className="secondary-button" type="button" disabled={busy} onClick={() => void act(() => retryCodingTask(selected.id))}><RefreshCw size={15} /> Повторить</button>}{selected.status === "review_ready" && <button className="primary-button" type="button" disabled={busy} onClick={() => void act(() => applyCodingTask(selected.id))}><Check size={16} /> {selected.project_root ? "Применить изменения" : "Подтвердить результат"}</button>}</div></header>
        {selected.error_text && <div className="notice is-error"><CircleAlert size={17} /> {selected.error_text}</div>}
        {selected.status === "waiting_for_input" && <div className="notice"><CircleAlert size={17} /> {String(selected.result.question ?? "Агент ожидает уточнения.")}</div>}
        {selected.result.summary !== undefined && <div className="coding-result"><h3>Результат</h3><p>{String(selected.result.summary)}</p>{selected.result.tests !== undefined && <pre>{String(selected.result.tests)}</pre>}</div>}
