  ...
  return (
    <section className="coding-page" ref={containerRef} aria-labelledby="coding-agent-title">
      <header className="coding-page-header">
        ...
        <button className="secondary-button" type="button" onClick={(e) => { animateButtonPress(e.currentTarget); void refresh(true); }} disabled={busy}><IconInterfaceSpirals size={16} /> Обновить среду</button>
      </header>
      {error && <div className="notice is-error"><IconInterfaceAlertAlarmBell2 size={17} /> {error}</div>}
      ...
      {selected && <section className="coding-detail" ref={detailRef}><header><div><span className={`coding-task-status status-${selected.status}`}>{statusLabel(selected.status)}</span><h2 data-i18n-skip>{selected.objective}</h2><small>{selected.model} · {selected.project_root || "отдельная рабочая папка"}</small>{selected.workspace_path && <small><span>Изолированный workspace: </span><span data-i18n-skip>{selected.workspace_path}</span></small>}</div><div className="coding-actions">{ACTIVE_TASKS.has(selected.status) && <button className="danger-button" type="button" disabled={busy} onClick={() => void act(() => cancelCodingTask(selected.id))}><IconInterfaceTimeStopWatchCircle size={15} /> Остановить</button>}{["failed", "cancelled", "waiting_for_input", "conflicted"].includes(selected.status) && <button className="secondary-button" type="button" disabled={busy} onClick={() => void act(() => retryCodingTask(selected.id))}><IconInterfaceSpirals size={15} /> Повторить</button>}{selected.status === "review_ready" && <button className="primary-button" type="button" disabled={busy} onClick={() => void act(() => applyCodingTask(selected.id))}><IconInterfaceFavoriteLike1 size={16} /> {selected.project_root ? "Применить изменения" : "Подтвердить результат"}</button>}</div></header>
        {selected.error_text && <div className="notice is-error"><IconInterfaceAlertAlarmBell2 size={17} /> <span data-i18n-skip>{selected.error_text}</span></div>}
        {selected.status === "waiting_for_input" && <div className="notice"><IconInterfaceAlertAlarmBell2 size={17} /> {selected.result.question !== undefined ? <span data-i18n-skip>{String(selected.result.question)}</span> : "Агент ожидает уточнения."}</div>}
        ...
        {selected.patch_text && <details className="coding-diff" open><summary><IconInterfaceFilesFolderCopy2 size={17} /> Diff и изменённые файлы</summary><pre>{selected.patch_text}</pre></details>}
        <div className="coding-detail-grid"><section><h3><IconProgrammingScript2 size={17} /> Логи, команды и ошибки</h3><div className="coding-events">{selected.events.length === 0 && <p className="muted">Событий ещё нет.</p>}{selected.events.map((item) => <article key={item.id} className={`event-level-${item.level}`}><header><strong>{item.message}</strong><time>{new Date(item.created_at).toLocaleTimeString(interfaceIntlLocale())}</time></header>{Object.keys(item.payload).length > 0 && <pre>{JSON.stringify(item.payload, null, 2)}</pre>}</article>)}</div></section><section><h3><IconMailSendEnvelope size={17} /> Вмешательство</h3><form onSubmit={submitInstruction}><textarea value={instruction} onChange={(event) => setInstruction(event.target.value)} placeholder="Дополнительная инструкция для задачи" disabled={busy || !ACTIVE_TASKS.has(selected.status)} /><button className="secondary-button" type="submit" disabled={busy || !ACTIVE_TASKS.has(selected.status) || !instruction.trim()}>Отправить инструкцию</button></form><p className="coding-hint">Уточнение доставляется на следующем шаге. Если агент ждёт решения, задача автоматически продолжится.</p></section></div>
      </section>}
    </section>
  );
}
