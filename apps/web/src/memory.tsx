...
export function MemoryPage() {
  ...
  return <section className="panel memory-panel">
    ...
    <div className="memory-list">
      ...
      {section === "commitments" && commitments.map((commitment) => <article className="memory-card" key={commitment.id}><div className="memory-card-main"><div className="memory-card-heading"><span className={`memory-status ${commitment.status === "open" ? "active" : "deleted"}`}>{commitment.status}</span></div><strong>{commitment.title}</strong><p>{commitment.details}</p><small>{commitment.kind} · уверенность: {Math.round(commitment.confidence * 100)}%</small></div></article>)}
      {section === "diagnostics" && <>
