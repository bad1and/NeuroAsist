...
export function StatePage({ events: liveEvents = [] }: { events?: Array<{ type: string }> }) {
  ...
  const removeReflection = async (id: string) => { if (!window.confirm("Удалить эту субъективную заметку Iris?")) return; await deleteCharacterReflection(id); await refresh(); };
  return <section className="state-page" aria-label="Состояние Iris">
    <header className="page-header"><div><span className="eyebrow">Внутренняя жизнь</span><h1>Состояние Iris</h1><p>{state ? `Сейчас: ${state.mood.primary_emotion}` : "Загружаем состояние…"}</p></div><button className="secondary" onClick={() => void refresh()}>Обновить</button></header>
    ...
    {state && <><div className="state-grid"><StateCard title="Настроение" data={state.mood} keys={["primary_emotion", "expression_strength", "secondary_emotions"]}/><StateCard title="Отношения" data={state.relationship} keys={["familiarity_label", "trust_label", "warmth_label", "tension_label", "playfulness_label"]}/><StateCard title="Динамика" data={state.relationship} keys={["current_dynamic", "unresolved_cause"]}/></div>
      ...
      <div className="state-actions"><button className="secondary" onClick={() => void reset("mood")}>Сбросить настроение</button><button className="secondary danger-button" onClick={() => void reset("relationship")}>Сбросить отношения</button></div></>}
    <section className="state-card"><h2>Недавние события</h2>{events.length ? <ul className="state-list">{events.map(event => <li key={event.id}><strong>{event.event_kind.split("_").join(" ")}</strong><span>{new Date(event.created_at).toLocaleString("ru-RU")}</span></li>)}</ul> : <p>Значимых событий пока нет.</p>}</section>
    <section className="state-card"><div className="panel-header"><div><h2>Личные заметки Iris</h2><span>Субъективные внутренние заметки Iris, а не факты о вас.</span></div><label className="settings-checkbox"><input type="checkbox" checked={reflectionEnabled} onChange={(event) => void toggleReflections(event.target.checked)}/>Создавать</label></div>{!reflectionEnabled ? <p>Создание новых личных заметок отключено.</p> : reflections.length ? <ul className="state-list">{reflections.map(item => <li key={item.id}><div><strong>{item.trigger_label}</strong><span>{item.text}</span><small>{item.primary_emotion} · {new Date(item.created_at).toLocaleString("ru-RU")}</small></div><button className="text-button" onClick={() => void removeReflection(item.id)}>Удалить</button></li>)}</ul> : <p>Пока нет субъективных заметок Iris.</p>}</section>
  </section>;
}
